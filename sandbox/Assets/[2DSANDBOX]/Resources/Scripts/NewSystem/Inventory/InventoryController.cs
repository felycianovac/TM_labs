...
public class InventoryController : MonoBehaviour,  ISaveable
{
  ...
  public Stack GetStuckById(int id)
  {
	var stack = inventory.items.FirstOrDefault(x => x.item.id == id);
	return stack;
  }
		...
		if (_open)
		{
			...
			for (int i = 0; i < items.Count; i++)
			{
				...
				if (GUI.Button(rect, ""))
				{
					...
					{
						GameObject.Find("Canvas").GetComponent<PauseScreen>()._player._placer.SetActiveItemIndex(i);
					}
				}
