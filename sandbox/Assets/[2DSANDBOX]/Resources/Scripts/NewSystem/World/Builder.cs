...
public class Builder
{
    ...
    public void buildMesh(Chunk chunk, bool isEditor)
    {
        reset();
        chunk.RemoveLights();
        ...
        for(int i = 0; i < _dimension; i++)
        {
            for(int j = 0; j < _dimension; j++)
            {
                ...
                Vector2 worldPos = new Vector2(_chunk.position.x * Chunk.tileDimension + pos.x, _chunk.position.y * Chunk.tileDimension + pos.y);
                ...
                if (tile.visible)
                {
                    if (tile.ai && !isEditor)
                    {
                        if (!chunk.IsAiSpawned(pos))
                        {
                            chunk.AddSpawnedAi(pos);
                            GameObject ai = GameObject.Instantiate<GameObject>(tile.ai, new Vector3(worldPos.x, worldPos.y, 1.0f), Quaternion.identity);
                            ai.GetComponent<AI>().tile = tile;
                        }
                    }
                    else
                    {
                        ...
                        if (tile.light != null)
                        {
                            chunk.AddLight(GameObject.Instantiate<Light>(tile.light, new Vector3(worldPos.x, worldPos.y, 1.0f), Quaternion.identity));
                        }
                    }
                }
