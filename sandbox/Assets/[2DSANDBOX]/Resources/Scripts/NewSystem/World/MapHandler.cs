...
public class MapHandler
{
    ...
    /// </summary>
    private int _loadedDimension = 5;
    private int _worldLength = 16;
    private int _worldHeight = 8;
    ...
    public MapHandler(Vector2I startingPosition, string mapName)
    {
        _mapName = mapName;
        ...
        for (int i = 0; i < _savedPerFrame; i++)
        {
            if (_saving.Count > 0)
            {
                ...
                else
                {
                    ...
                    MapLoader.saveSector(sector, _mapName);
                }
            }
    ...
    private void forceQueue()
    {
        for (int i = 0; i < _saving.Count; i++)
        {
            ...
            MapLoader.saveSector(s, _mapName);
        }
    }
    ...
    private Sector getSector(Vector2I pos)
    {
        ...
        if (MapLoader.sectorExists(pos, _mapName))
        {
            sector = MapLoader.loadSector(pos, _mapName);
        }
        else // the sector has not yet been generated, queue it for generation
        {
            sector = _generator.generateSector(pos.x, pos.y);
