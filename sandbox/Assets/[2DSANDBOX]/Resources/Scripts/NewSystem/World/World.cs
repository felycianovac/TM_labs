...
public class World : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if (DataManager.isMultiplayer)
        {
            ...
            mapNames[0] = DataManager.multiplayerMapName;
        }
        else
        {
        ...
        mapName = mapNames[Random.Range(0, mapNames.Length)];
        ...
        //if(!DataManager.isMultiplayer || PhotonNetwork.isMasterClient)
        //{
        createWorld();
        //}

        createLoader(spawnPos, mapName);
    ...
    //Where Player spawn in map
    public Vector2 PositionFromMapName(string mapName)
    {
        switch (mapName)
        {
            case "Map2":
                //He spawn in 0x and 22y in Map2
                return new Vector2(10, 22);
            default:
                return new Vector2(0, 22);
        }
    }

    private void createLoader(Vector2I position, string mapName)
    {
        if (DataManager.isMultiplayer)
        {
            int half = (_properties.worldDimension - 1) / 2;
            ...
            Vector2 playerPos = PositionFromMapName(mapName);
            //_playerObj = GameObject.Instantiate(loadedPlayer, new Vector3(playerPos.x, playerPos.y, 1.0f), Quaternion.identity);
            _playerObj = PhotonNetwork.Instantiate(PlayerPrefs.GetString("PlayerAsset", "Player"), new Vector3(playerPos.x, playerPos.y, 1.0f), Quaternion.identity, 0);
    ...
    public void createWorld()
    {
        ...
        for (int i = 0; i < dimension; i++)
        {
            for (int j = 0; j < dimension; j++)
            {
                ...
                ////////PhotonView tempView = obj.AddComponent<PhotonView>();
                ////////tempView.viewID = viewIDNum;
                ////////viewIDNum += 1;
                //StartCoroutine(AddViewObserve(tempView));
                ////////if (tempView.ObservedComponents == null)
                ////////{
                ////////    tempView.ObservedComponents = new List<Component>();
                ////////}
                ////////tempView.ObservedComponents.Add(tempView.gameObject.GetComponent<ChunkWorldObject>());
                ////////tempView.synchronization = ViewSynchronization.ReliableDeltaCompressed;
    ...
    //IEnumerator AddViewObserve(PhotonView tempView)
    //{
    //    if (tempView.ObservedComponents == null)
    //    {
    //        tempView.ObservedComponents = new List<Component>();
    ...
    //    tempView.ObservedComponents.Add(tempView.gameObject.GetComponent<ChunkWorldObject>());
    //    Debug.Log((bool)tempView.gameObject.GetComponent<ChunkWorldObject>());
    //}

    public void SetBlockForce(Tile tile, Vector2 position)
    {
        ...
        var currentChunk = GetChunk(chunkPosition);
        if (currentChunk != null)
            currentChunk.setTile(tile, blockPosition);
    }

    /*public void UpdateBackgroundBlock(Vector2 position)
    {
      position += new Vector2(0.5f, 0.5f);

      var chunkPosition = chunkPositionFromWorld(position);
      var oldChunk = GetChunk(chunkPosition);

      var old = oldChunk.getTile(chunkPosition.x, chunkPosition.y);

      oldChunk.setBackgroundTile(old, chunkPosition);
    }*/



    /************************/
    public void UpdateBlockForce(Tile tile, Vector2 position)
    {
        position += new Vector2(0.5f, 0.5f);

        var chunkPosition = chunkPositionFromWorld(position);
        var blockPosition = blockFromChunk(chunkPosition, position);
    ...
    {
        var oldchunkPosition = chunkPositionFromWorld(position);
        var oldblockPosition = blockFromChunk(oldchunkPosition, position);

        var globalPos = BlockUtil.localBlockToWorldBlock(new Vector2I(oldblockPosition.x, oldblockPosition.x)
                                                         , new Vector2I(oldchunkPosition.x, oldchunkPosition.y));
        var backTile = globalPos.y < 20 ? new Tile(6, 0) : new Tile(0, 0);


        setBackground(backTile, position, false);
        setBlock(new Tile(0, 0), position, false);
    }

    /************************/

    public void setBlock(Tile tile, Vector2 position, bool isDrop = true) //Removing Block step 2 ani
    {
        position += new Vector2(0.5f, 0.5f);
        ...
        old.decreaseHealth(pauseScreen.GetToolId());  //Removing Block step 3 (decreasing block health) ani

        if (old.health <= 0)
        {
            if (isDrop)
                old.onDestroyed(position);   //Removing Block step 4 (Instantiating pickable block) ani
            chunk.setTile(tile, blockPosition);   //Removing Block step 5 (removig block) ani
        }
        ...
        /*for (int i = 0; i < _activeChunks.Count; i++)
        {
          ...
          if (chunk != null)
          {
            ...
            if (chunk.position == chunkPosition)
            {
              ...
              old.decreaseHealth(pauseScreen.GetToolId());
    ...
    private Chunk GetChunk(Vector2I position)
    {
        var worldObject = _activeChunks.FirstOrDefault(x => x.chunk != null && x.chunk.position == position);
        return worldObject == null ? null : worldObject.chunk;
    }

    public void setBackground(Tile tile, Vector2 position, bool isDrop = true)
    {
        position += new Vector2(0.5f, 0.5f);
    ...
    public Tile getTile(Vector2 position)
    {
        ...
        {
            Chunk chunk = _activeChunks[i].chunk;
            //Debug.Log("chunk position " + chunk.position);
            if (chunk != null)
            {
                // Found the correct chunk?
                if (chunk.position == chunkPosition)
